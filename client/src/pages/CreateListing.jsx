...
const handleSubmit = async (e) => {
  e.preventDefault();
  try {
    if(formData.imageURLs.length < 1) return setError('you must upload at laest one image');
    if(+formData.regularPrice < +formData.discountPrice) return setError('discount price must be less than regular price');
    setLoading(true);
    setError(false);
    console.log(currentUser._id);  // Check if the ID is correctly available
    const res = await fetch('http://localhost:3000/api/listing/create', {
      ...
      body: JSON.stringify({
        ...formData,
        useRef: currentUser?._id,  // Ensure _id is being sent
    }),
    });
    ...
    <div className="p-3 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <button disabled={loading || Uploading}  className="p-3 bg-slate-700 text-white rounded-l uppercase hover:opacity-95 disabled:opacity-80">
